...
describe('PUT /api/v1/posts/:id/like', () => {
  ...
  it('should like a post for authenticated user', async () => {
    const registerResponse = await request(app).post('/api/v1/auth/register').send({
      username: 'dodger_like_user',
      email: 'dodger_like_test@example.com',
  ...
  it('should remove like on second toggle', async () => {
    const registerResponse = await request(app).post('/api/v1/auth/register').send({
      username: 'dodger_like_user',
      email: 'dodger_like_test@example.com',
      password: 'strong_password_123',
  ...
  it('should return 401 when liking a post without token', async () => {
    const registerResponse = await request(app).post('/api/v1/auth/register').send({
      username: 'dodger_like_user',
      email: 'dodger_like_test@example.com',
      password: 'strong_password_123',
    ...
    const postResponse = await request(app)
      ...
      .set('Authorization', `Bearer ${token}`)
      .send({
        desc: 'Post for unauthorized like testing',
      });

    const postId = postResponse.body.id as number;

    const response = await request(app).put(`/api/v1/posts/${postId}/like`);
